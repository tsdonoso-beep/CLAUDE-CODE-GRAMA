# MEGA PROMPT — GRAMA LXP: Plataforma de Capacitación Docente TSF
...
Estás construyendo **GRAMA LXP** (Learning Experience Platform), una plataforma híbrida de capacitación docente para los **9 talleres de Educación Para el Trabajo (EPT)** del programa **MSE-SFT / TSF de MINEDU Perú**, desarrollada por **GRAMA Proyectos Educativos**.
...
- localStorage para persistencia de progreso del docente
...
├── contexts/
│   └── ProgressContext.tsx     (progreso global del docente en localStorage)
├── data/
...
├── hooks/
│   ├── useProgress.ts          (leer/escribir progreso en localStorage)
│   └── useTaller.ts            (helper para obtener datos del taller activo)
├── pages/
│   ├── Bienvenida.tsx          (pantalla pública con los 9 talleres)
...
interface ContenidoItem {
  ...
  completado?: boolean       // gestionado por ProgressContext
}
```
...
4. Al completar un ítem: marcar como ✅ (escribe en ProgressContext → localStorage)
...
## CONTEXTO DE PROGRESO — `ProgressContext.tsx`

```typescript
interface ProgressState {
  // Por taller y por contenido
  completados: Record<string, Set<string>>     // tallerSlug → Set de contenidoIds completados
  quizResultados: Record<string, number>       // contenidoId → puntaje (0-100)
  modulosBloqueados: Record<string, Set<number>> // tallerSlug → Set de números de módulos bloqueados
  perfilGrado: Record<string, number>          // tallerSlug → grado seleccionado (1-5)
  ultimaActividad: Record<string, Date>        // tallerSlug → última vez activo
}

// Reglas de desbloqueo:
// - M0 siempre disponible
// - M1 siempre disponible  
// - M2 y M3 se bloquean si Quiz Seguridad (M1.1.6) < 80%
// - M4 disponible si M3 tiene al menos 1 actividad práctica completada
// - M5 disponible si M4 Quiz >= 75%
// - M6 disponible si M5 Sub-sección 5.10 (planificación) completada
```

**Persiste en localStorage**:
```typescript
const STORAGE_KEY = 'grama-lxp-progress-v1'
// Serializar/deserializar Sets como arrays
```
...
3. Si aprueba (>= puntajeMinimo): verde, desbloquea siguiente, escribe en ProgressContext
...
6. **ProgressContext** debe inicializarse con todos los módulos del M0 y M1 como disponibles (no bloqueados)
...
5. **No ignorar los estados de bloqueo** — el Quiz de Seguridad debe realmente bloquear M2 y M3
6. **No mezclar la lógica de progreso con la UI** — usar ProgressContext para todo el estado

### Orden de construcción recomendado

```
1. tailwind.config.ts + globals.css (tokens GRAMA)
2. ProgressContext.tsx
3. modulosLXP.ts (data completa de los 7 módulos)
4. Componentes base: ContenidoBadge, ProgressRing, ModuloCard
5. AppShell + Sidebar
6. Página Bienvenida (/)
7. TallerHub (/taller/:slug)
8. RutaAprendizaje (/taller/:slug/ruta)
9. ModuloDetalle (/taller/:slug/ruta/modulo/:num) con QuizBlock
10. Repositorio (/taller/:slug/repositorio)
11. BienDetalle (/taller/:slug/repositorio/bien/:id)
12. Revisión de consistencia visual GRAMA en todas las páginas
```

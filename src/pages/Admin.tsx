...
Equipo GRAMA · Programa TSF-MINEDU`
  ...
  const selectStyle = {
    ...
    style: { borderColor: 'rgba(255,255,255,0.75)', background: '#ffffff', color: '#043941' },
  }
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: "'Manrope', sans-serif" }}>
      ...
      {/* ── Sidebar ─────────────────────────────────────────────────────────── */}
      <aside style={{ width: 240, background: '#032d34', position: 'fixed', top: 0, left: 0, bottom: 0, zIndex: 40, display: 'flex', flexDirection: 'column', borderRight: '1px solid rgba(255,255,255,0.80)' }}>

        {/* Logo */}
        <div style={{ padding: '1.25rem 1.1rem .9rem', borderBottom: '1px solid rgba(4,57,65,0.12)' }}>
          <GramaLogo variant="light" size="sm" />
          <p style={{ fontSize: '.56rem', color: 'rgba(255,255,255,0.6)', fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase', marginTop: '.45rem' }}>Panel de administración</p>
        </div>
        ...
        <nav style={{ flex: 1, overflowY: 'auto', padding: '.6rem .55rem' }}>
          {NAV_GROUPS.map(group => (
            <div key={group.label} style={{ marginBottom: '1.1rem' }}>
              <p style={{ fontSize: '.56rem', fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.75)', padding: '.3rem .6rem .4rem' }}>{group.label}</p>
        ...
        {/* User footer */}
        <div style={{ padding: '.8rem 1rem', borderTop: '1px solid rgba(255,255,255,0.80)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '.6rem', marginBottom: '.65rem' }}>
            ...
            <div style={{ minWidth: 0 }}>
              <p style={{ fontSize: '.72rem', fontWeight: 700, color: 'rgba(255,255,255,0.9)' }}>Admin</p>
              <p style={{ fontSize: '.6rem', color: 'rgba(255,255,255,0.7)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 140 }}>{profile?.email}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
              {([
                ...
                { label: 'Solicitudes pendientes',value: solicitudesPendientes,      icon: Inbox,         color: '#043941' },
                ...
                return (
                  <div key={d.id} style={{ background: '#ffffff', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '1.1rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '.65rem', marginBottom: '.75rem' }}>
                      ...
                      <span style={{ marginLeft: 'auto', fontSize: '.8rem', fontWeight: 800, color: pct >= 70 ? '#02d47e' : pct >= 30 ? '#f59e0b' : 'rgba(255,255,255,0.75)', flexShrink: 0 }}>{pct}%</span>
                    </div>
        ...
        {tab === 'solicitudes' && (
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
              <div>
                ...
                <p style={{ fontSize: '.72rem', color: 'rgba(4,57,65,0.6)' }}>
                  {solicitudesPendientes > 0 ? <span style={{ color: '#043941', fontWeight: 700 }}>{solicitudesPendientes} pendientes</span> : 'Sin pendientes'} · {solicitudes.length} en total
                </p>
              </div>
              <button onClick={fetchSolicitudes} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '.72rem', fontWeight: 600, padding: '.4rem .9rem', borderRadius: 9, background: '#ffffff', color: 'rgba(4,57,65,0.75)', border: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer' }}>
                ...
                return (
                  <button key={estado} onClick={() => setFiltroEstado(estado)}
                    style={{ padding: '.38rem .9rem', borderRadius: 20, fontSize: '.75rem', fontWeight: 700, cursor: 'pointer', border: 'none', transition: 'all .15s',
                      ...
                      color:      active ? '#043941' : 'rgba(255,255,255,0.75)' }}>
            ...
            <div className="flex items-start gap-3 px-5 py-4 rounded-2xl mb-6"
              ...
              <AlertTriangle size={16} style={{ color: '#043941', flexShrink: 0, marginTop: 1 }} />
              <div>
                <p className="text-sm font-bold" style={{ color: '#043941' }}>Información sensible</p>
            ...
            {loadingSolicitudes ? (
              ...
              <div className="text-center py-20 rounded-2xl"
                style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.80)' }}>
                <Inbox size={36} className="mx-auto mb-3" style={{ color: 'rgba(4,57,65,0.75)' }} />
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2 mb-1">
                            ...
                            <span className="text-xs px-2 py-0.5 rounded-lg font-bold"
                              style={sol.estado === 'pendiente'
                                ? { background: 'rgba(245,158,11,0.15)', color: '#043941' }
                                : sol.estado === 'aprobado'
                          ...
                          {sol.mensaje && (
                            <p className="mt-2 text-xs px-3 py-2 rounded-lg italic"
                              style={{ background: '#ffffff', color: '#043941', border: '1px solid rgba(255,255,255,0.80)' }}>
                              "{sol.mensaje}"
                            </p>
                          )}
                          {password && (
                            <div className="mt-3 space-y-2">
                              <div className="flex items-center gap-2 px-3 py-2 rounded-lg"
                                ...
                                <code className="flex-1 text-sm font-bold" style={{ color: '#043941', letterSpacing: '0.05em' }}>{password}</code>
                                <button
                                  onClick={() => {
                                  ...
                                  style={{ background: 'rgba(245,158,11,0.2)', color: '#043941' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
                <div>
                  ...
                  <p style={{ fontSize: '.72rem', color: 'rgba(4,57,65,0.6)' }}>
                    {consultasPendientes > 0 ? <span style={{ color: '#043941', fontWeight: 700 }}>{consultasPendientes} sin responder</span> : 'Todas respondidas'} · {consultasAdmin.length} en total
                  </p>
                </div>
                <button onClick={fetchConsultasAdmin} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '.72rem', fontWeight: 600, padding: '.4rem .9rem', borderRadius: 9, background: '#ffffff', color: 'rgba(4,57,65,0.75)', border: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer' }}>
                  <RefreshCw size={12} /> Recargar
                </button>
              </div>
              {/* Filtros */}
              ...
              {loadingConsultas ? (
                ...
                <div className="text-center py-20 rounded-2xl"
                  style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.80)' }}>
                  <MessageCircle size={36} className="mx-auto mb-3" style={{ color: 'rgba(4,57,65,0.75)' }} />
                    ...
                    return (
                      <div key={c.id} className="rounded-2xl overflow-hidden"
                        ...
                        <div className="px-5 py-4">
                          ...
                          <p className="text-sm leading-relaxed" style={{ color: 'rgba(4,57,65,0.8)' }}>{c.mensaje}</p>
                        </div>
                        ...
                        {c.respuesta && (
                          <div className="px-5 py-4 border-t"
                            ...
                            <p className="text-sm leading-relaxed" style={{ color: 'rgba(4,57,65,0.7)' }}>{c.respuesta}</p>
                          </div>
                        )}
                        ...
                        {!respondida && (
                          <div className="px-5 py-4 border-t space-y-3"
                            style={{ borderColor: 'rgba(255,255,255,0.80)', background: 'rgba(255,255,255,0.02)' }}>
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    style={{ borderColor: filtroAnalyticsDocente ? '#02d47e' : 'rgba(255,255,255,0.75)', background: '#ffffff', color: '#043941' }}>
                    <option value="" style={{ background: '#0d2b31', color: '#043941' }}>Todos los docentes (global)</option>
                    {docentes.map(d => (
                      <option key={d.id} value={d.id} style={{ background: '#0d2b31', color: '#043941' }}>{d.nombre_completo} — {d.email}</option>
                    ))}
                  </select>
                          ...
                          <div key={row.label}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '.45rem', width: 192, flexShrink: 0 }}>
                                {row.badge ? (
                                  <span style={{ fontSize: '.58rem', fontWeight: 800, background: isBase ? 'rgba(255,255,255,0.12)' : '#02d47e', color: isBase ? 'rgba(255,255,255,0.75)' : '#043941', padding: '.15rem .4rem', borderRadius: 5, flexShrink: 0 }}>
                                    {row.badge}
                                  </span>
                                ) : (
                                  <span style={{ fontSize: '.58rem', fontWeight: 800, background: '#ffffff', color: 'rgba(4,57,65,0.75)', padding: '.15rem .45rem', borderRadius: 5, flexShrink: 0 }}>ALL</span>
                                )}
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '.6rem', flexShrink: 0 }}>
                                <span style={{ fontSize: '.78rem', fontWeight: 800, color: isBase ? 'rgba(255,255,255,0.75)' : '#fff', width: 28, textAlign: 'right' }}>{row.count}</span>
                                <span style={{ fontSize: '.72rem', fontWeight: 700, color: isBase ? 'rgba(255,255,255,0.75)' : barColor, width: 38, textAlign: 'right' }}>{pct}%</span>
                              </div>
                            </div>
                          </div>
                        )
              ...
              return (
                <div style={{ borderRadius: 16, overflow: 'hidden', marginBottom: '2rem', border: '1px solid rgba(255,255,255,0.08)' }}>
                  ...
                  {grupos.map((g, gi) => (
                    <div key={g.modulo}>
                      {/* group header */}
                      <div style={{ padding: '.45rem 1.5rem', background: 'rgba(255,255,255,0.03)',  borderBottom: '1px solid rgba(4,57,65,0.07)', borderTop: gi > 0 ? '1px solid rgba(255,255,255,0.80)' : 'none', display: 'flex', alignItems: 'center', gap: '.5rem' }}>
                      ...
                      {g.items.map((q, qi) => {
                        ...
                        return (
                          <div key={q.contenidoId} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 130px 70px', gap: 0, padding: '.75rem 1.5rem', borderBottom: isLast ? 'none' : '1px solid rgba(255,255,255,0.04)', alignItems: 'center' }}>
                            ...
                            {/* intentos */}
                            <p style={{ fontSize: '.78rem', fontWeight: 700, color: 'rgba(4,57,65,0.6)' }}>{q.intentos}</p>
                            {/* docentes */}
                            <p style={{ fontSize: '.78rem', fontWeight: 700, color: 'rgba(4,57,65,0.6)' }}>{q.docentesUnicos}</p>
                            {/* tasa bar */}
                            ...
                            <div style={{ display: 'flex', gap: '.25rem', flexWrap: 'wrap' }}>
                              ...
                              {q.puntajeMinimo && <span style={{ fontSize: '.55rem', fontWeight: 800, background: 'rgba(245,158,11,0.15)', color: '#043941', padding: '.1rem .3rem', borderRadius: 4 }}>MÍN {q.puntajeMinimo}%</span>}
                            </div>
                          </div>
                        )
                      })}
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    { label: 'Videos reproducidos', value: analytics.contenidos.filter(c => c.tipo_evento === 'reproduccion_video').reduce((a, c) => a + c.count, 0), Icon: Video, color: '#043941' },
                    ...
                    {analytics.navegacion.map((n, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center gap-3">
                          ...
                          <span className="text-xs font-bold w-8 text-right" style={{ color: 'rgba(4,57,65,0.6)' }}>{n.count}</span>
                        </div>
                      )
                    })}
                  </div>
                      ...
                      {([7, 14, 30] as const).map(d => (
                        <button key={d} onClick={() => setFiltroInactividad(d)}
                          style={{ fontSize: '.68rem', fontWeight: 700, padding: '.28rem .7rem', borderRadius: 20, cursor: 'pointer', border: 'none', transition: 'all .15s',
                            ...
                            color: filtroInactividad === d ? '#043941' : 'rgba(255,255,255,0.75)',
                          }}>
                          +{d}d
                        </button>
                      ))}
                  ...
                  {inactivos.length > 0 && (
                    <div>
                      {inactivos.map((d, i) => {
                        ...
                        const diasColor = dias === null || dias > 60 ? '#ef4444' : dias > 30 ? '#f59e0b' : dias > 14 ? 'rgba(255,255,255,0.75)' : 'rgba(255,255,255,0.35)'
                        ...
                        return (
                          <div key={d.id}
                            ...
                            {/* avatar */}
                            <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(245,158,11,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.65rem', fontWeight: 800, color: '#043941', flexShrink: 0 }}>
                              {initials}
                            </div>
                            {/* name + email */}
        ...
        <div className="flex flex-col gap-3 mb-5">
          ...
          <div className="relative">
            ...
            <input
              ...
              style={{ borderColor: busquedaUsuario ? '#02d47e' : 'rgba(255,255,255,0.12)', background: '#ffffff', color: '#043941' }}
            />
            {busquedaUsuario && (
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex gap-3 flex-wrap">
              <select value={filtroIE} onChange={e => setFiltroIE(e.target.value)} {...selectStyle}>
                <option value="" style={{ background: '#0d2b31', color: '#043941' }}>Todas las IEs</option>
                {INSTITUCIONES_EDUCATIVAS.map(ie => (
                  <option key={ie.id} value={String(ie.id)} style={{ background: '#0d2b31', color: '#043941' }}>{ie.nombre}</option>
                ))}
              </select>
              <select value={filtroTaller} onChange={e => setFiltroTaller(e.target.value)} {...selectStyle}>
                <option value="" style={{ background: '#0d2b31', color: '#043941' }}>Todos los talleres</option>
                {talleresEnUso.map(slug => {
                  const t = talleresConfig.find(t => t.slug === slug)
                  return <option key={slug} value={slug!} style={{ background: '#0d2b31', color: '#043941' }}>{t?.nombre ?? slug}</option>
                })}
              </select>
            </div>
          </div>
                ...
                <tbody>
                  {docentesFiltrados.map((d, i) => {
                    ...
                    return (
                      <tr key={d.id}
                        onClick={() => setDocenteDetalle(d)}
                        style={{ borderBottom: i < docentesFiltrados.length - 1 ? '1px solid rgba(255,255,255,0.80)' : 'none', cursor: 'pointer' }}
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold" style={{ color: 'rgba(4,57,65,0.7)' }}>
                              {d.porcentaje}%
                            </span>
                          </div>
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="rounded-xl p-4 space-y-3" style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)' }}>
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-xs" style={{ color: '#043941' }}>Contraseña</span>
                        <div className="flex items-center gap-2">
                          <code className="text-sm font-bold" style={{ color: '#043941', letterSpacing: '0.05em' }}>{usuarioCreadoOk.password}</code>
                          <button
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input value={nuevoPassword} onChange={e => setNuevoPassword(e.target.value)}
                          ...
                          style={{ background: '#ffffff', color: '#043941', border: '1px solid rgba(245,158,11,0.25)', letterSpacing: '0.05em' }} />
                        <button onClick={() => setNuevoPassword(generatePassword())}
                          className="px-3 py-2 rounded-xl text-xs font-semibold shrink-0"
                          style={{ background: 'rgba(245,158,11,0.12)', color: '#043941', border: '1px solid rgba(245,158,11,0.2)' }}>
                          Regenerar
                        </button>
                      </div>
                    </div>
                      ...
                      <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.1)', maxHeight: 180, overflowY: 'auto' }}>
                        {talleresConfig.map((t, i) => (
                          <label key={t.slug}
                            ...
                            style={{ borderBottom: i < talleresConfig.length - 1 ? '1px solid rgba(255,255,255,0.80)' : 'none' }}>
              ...
              <div className="shrink-0" style={{ borderBottom: '1px solid rgba(4,57,65,0.12)', padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', gap: '.85rem' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '.4rem', marginTop: '.18rem' }}>
                    ...
                    {taller && <span style={{ fontSize: '.62rem', color: 'rgba(4,57,65,0.62)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{taller.nombreCorto ?? taller.nombre}</span>}
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto" style={{ padding: '1.25rem' }}>
                ...
                {ie && (
                  <div style={{ background: '#ffffff', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '.75rem 1rem', marginBottom: '1.1rem' }}>
                    ...
                    <p style={{ fontSize: '.68rem', color: 'rgba(4,57,65,0.62)', marginTop: '.18rem' }}>{ie.distrito} · {ie.region}</p>
                  </div>
                )}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '.6rem', marginBottom: '1rem' }}>
                  {[
                    ...
                    <div key={s.label} style={{ background: '#ffffff', border: '1px solid rgba(255,255,255,0.80)', borderRadius: 11, padding: '.65rem .5rem', textAlign: 'center' }}>
                      <p style={{ fontSize: '1.2rem', fontWeight: 900, color: s.color, lineHeight: 1 }}>{s.value}</p>
                      <p style={{ fontSize: '.58rem', color: 'rgba(4,57,65,0.62)', marginTop: '.3rem' }}>{s.label}</p>
                    </div>
                  ))}
                </div>
                ...
                {d.visualizados > d.completados && (
                  <p style={{ fontSize: '.62rem', color: 'rgba(4,57,65,0.75)', marginBottom: '1rem' }}>
                    ...
                    <div className="rounded-xl p-4 space-y-3"
                      ...
                      <div className="flex items-center justify-between">
                        <p className="text-xs font-bold" style={{ color: '#043941' }}>
                          Talleres asignados
                          {hayPendientes && (
                            <span className="ml-2 text-[10px] font-extrabold" style={{ color: '#043941' }}>
                              · cambios sin guardar
                            </span>
                          )}
                        </p>
                      ...
                      {talleresDisponibles.length > 0 && (
                        <select
                          ...
                          style={{ background: '#ffffff', color: 'rgba(4,57,65,0.75)', border: '1px solid rgba(255,255,255,0.1)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '.4rem', marginBottom: '1.1rem' }}>
                  {[
                    ...
                    <div key={row.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <p style={{ fontSize: '.72rem', color: 'rgba(4,57,65,0.62)' }}>{row.label}</p>
                      <p style={{ fontSize: '.72rem', fontWeight: 600, color: 'rgba(4,57,65,0.6)' }}>{row.value}</p>
                    </div>
                  ))}
                </div>
                    ...
                    {passwordsReset[d.id] ? (
                      <>
                        <div className="flex items-center gap-2 px-3 py-2 rounded-lg"
                          ...
                          <code className="flex-1 text-sm font-bold" style={{ color: '#043941', letterSpacing: '0.05em' }}>
                            {passwordsReset[d.id]}
                          </code>
                          <button
                        ...
                        <button
                          ...
                          style={copiadoBienvenida === d.id
                            ...
                            : { background: '#ffffff', color: 'rgba(4,57,65,0.6)', border: '1px solid rgba(255,255,255,0.1)' }}>
